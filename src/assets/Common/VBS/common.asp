<%
dim gblStringToolTipPhoneNumber, gblStringToolTipPhoneNumberAUS, gblStringToolTipEmail, gblStringToolTipSSN, gblStringToolTipLastActualScheduledRentIncrease, gblStringToolTipNextScheduledRentIncrease, gblStringToolTipNextAnticipatedRentIncrease, gblStringToolTipNoChangeSOSIfNoQSCompleted
dim jsVersion, versionForIncludes
	
gblStringToolTipPhoneNumber="Enter the 10 digits of the phone number, no dashes."
gblStringToolTipPhoneNumberAUS="Enter the 10 digits of the phone number, no dashes."
gblStringToolTipEmail = "Please enter a valid email address.  Valid emails must include (@ and .) ."
...
gblStringToolTipNoChangeSOSIfNoQSCompleted = "Since no resident screening application was completed, the SOS may not be changed."

initVersionForIncludes()

sub initVersionForIncludes()
  dim objFileToRead
  Set objFileToRead = CreateObject("Scripting.FileSystemObject").OpenTextFile( Server.MapPath("/Common/VersionForIncludes.inc"),1)
  versionForIncludes = objFileToRead.ReadAll() '*** This will be used for all javascript and css includes.  So if you make an update to a javascript or css file, changing this value will force user's browsers to clear their cache of that file.  We may decide to pull this value from the database.
  objFileToRead.Close
...
Function myIsNumeric(value)
	If IsNull(value) Then
		myIsNumeric = false
	Else
		myIsNumeric = IsNumeric(CStr(value))
	End If
End Function

function formatDecimalsNoCommas(n,dec)
	if myIsNumeric(n) then
...
function IsEdgeBrowser()
	dim usr_agent, isBrowserEdge
	usr_agent = request.serverVariables("HTTP_USER_AGENT")
	isBrowserEdge = false
	if InStr(usr_agent,"Edge/") > 0 then
		isBrowserEdge = true
	end if
    IsEdgeBrowser = isBrowserEdge
end function
    
function IsFirefox()
	dim usr_agent, isBrowserFirefox
	usr_agent = request.serverVariables("HTTP_USER_AGENT")
	isBrowserFirefox = false
	if InStr(usr_agent,"Firefox/") > 0 then
...
function IsSafari()
	dim usr_agent, isBrowserSafari
	usr_agent = request.serverVariables("HTTP_USER_AGENT")
	isBrowserSafari = false
	if InStr(usr_agent,"Safari/") > 0 and  InStr(usr_agent,"Chrome/") <= 0 then
...
function InList(searchFor, listOfItems)
	'/////////////////////////////////////////////////////////////////
	'returns a boolean (TRUE OR FALSE) if the searchFor string is in the comma seporated items in the listOfItems argument
	'usage:
	'	'see if an item is in the list
	'	if InList("D","A,B,C,D,E") then
	'		response.write("it is IN the list")
	'	end if
	'	'see if an item is NOT in the list
	'	if not InList("Z","A,B,C,D,E") then
	'		response.write("it is NOT in the list")
	'	end if
	'/////////////////////////////////////////////////////////////////
    Dim arr
    arr = Split(listOfItems, ",")
    For Each s In arr
        If LCase(s) = LCase(searchFor) Then
            InList = True
            Exit Function
        End If
    Next
