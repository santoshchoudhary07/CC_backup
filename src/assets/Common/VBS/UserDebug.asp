...
' Existing comment delimiters are replaced with "{*" and "*}".
'
sub subDebugJavascriptComment(enabled, text)
...
'**************************************************************************************************
' fnDebugGetElapsedTimeText
'
'   tStart: timer() - the start time
'
' Use this method to build an elapsed time string for debug output.
' Example Return: 02:51:47.0963
'
function fnDebugGetElapsedTimeText(tStart)
  ...
  sText = right(sPartText, 2) & sText
  fnDebugGetElapsedTimeText = sText
end function
...
'**************************************************************************************************
' fnSetUserDebugFlagEnabled
'
'   companyId:  number or string - the company ID of the user to load debug flags for
'   userId: number or string - the ID of the user to check debug flag for
'   flag:   string - the flag to set the enabled state for
'
' Use this method to set the enabled state (bool) for the given "userId"'s debug "flag". As a
' convenience, if the user's debug flags have not already been loaded, this method will load
' them.
'
function fnSetUserDebugFlagEnabled(companyId, userId, flag, enabled)

  dim bOldEnabled, sKey
  ...
  if not dbgUserDebugFlags is nothing then
    sKey = trim(cstr(companyId)) & "_" & trim(cstr(userId)) & "_" & ucase(flag)
    if dbgUserDebugFlags.exists(sKey) then
      bOldEnabled = dbgUserDebugFlags.item(sKey)
      dbgUserDebugFlags.item(sKey) = enabled
    else
      dbgUserDebugFlags.add sKey, enabled
    end if
  end if

  fnSetUserDebugFlagEnabled = bOldEnabled
