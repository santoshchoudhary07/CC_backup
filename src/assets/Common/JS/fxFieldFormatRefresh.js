...
function fnFormatPhoneNumberAUS(oObj) {
	var sValue = ((oObj.value).toString()).replace(/[^0-9]/gi, '');
		...
		if (oObj.getAttribute('isDirty') == '1') {
			...
			if (trim(sValue) != '') {
				if ((sValue).length < 10) {
					...
					oObj.value = '(' + (sValue).substr(0, 2) + ')' + (sValue).substr(2, 4) + '-' + (sValue).substr(6, 4);
				}
			}
			else {
				oObj.value = '';
...
function fnFormatPhoneNumberAUSCell(oObj) {
	var sValue = ((oObj.value).toString()).replace(/[^0-9]/gi, '');
		...
		if (oObj.getAttribute('isDirty') == '1') {
			...
			if (trim(sValue) != '') {
				if ((sValue).length < 10) {
					...
					oObj.value = '(' + (sValue).substr(0, 4) + ')' + (sValue).substr(4, 3) + '-' + (sValue).substr(7, 3);
				}
			}
			else {
				oObj.value = '';
...
function fnEmailIsValidAndNotBlank(fldEmail, sDefaultResEmail, sAlert) {
    ...
    if (fldEmail.value == "") {
        if (sDefaultResEmail != "") {
            maUI.dialog("Alert", sAlert).then(function () {
                var $frm = $(fldEmail).parents("form:first");
                $frm.find("div.accordion section.accordion-section").addClass("accordion-section-expanded");
        ...
        if (!emailCheck(fldEmail.value)) {
            maUI.dialog("Alert", "Please note that you have entered an invalid email address into the \"Email\" field.  Please correct your entry before proceeding.").then(function () {
                var $frm = $(fldEmail).parents("form:first");
                $frm.find("div.accordion section.accordion-section").addClass("accordion-section-expanded");
                fldEmail.focus();
                fldEmail.select();
		...
		if (oObj.getAttribute('isDirty') == '1') {
			...
			if (trim(sValue) != '') {
				if (ssnCheck(sValue)) {
					...
					ssn = sValue;
					if (ssn.indexOf("-") == -1) {
						ssn = String(ssn).substring(0, 3) + "-" + String(ssn).substring(3, 6) + "-" + String(ssn).substring(6, 10);
					}
					oObj.value = ssn;
				}
				else {
