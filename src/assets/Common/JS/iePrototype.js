...
function MA_objectHasCapability(obj, propertyName){
    ...
    //edit: 2018-02-06 by bc. 
    //Object.getOwnPropertyNames does not seem to work well with built-in objects (like HTMLDocument)
    //it simply does not "see" them and so return a false "negative" when the propertyName is not found...
    //which directs the IE_PROTOTYPE_INIT function to "override" functions that may already exist!!!
    //this seems to have been a non-issue up until now... 
    //but Chrome/v64 has some READ-ONLY properties on the HTMLDocument object, so it was throwing an error.
    //changing to manually search via "keys" within the object's prototype (should work for any object)
    for(var k in obj.prototype) {
        if(k === propertyName) 
            return true; //found propertyName... so get out
    }
    return false; //nothing found, so propertyName not in the object
}
