...
sub SyncBOData(bo_application_id, from_ofs_api_log_id)
    dim bo_http_resolveTimeout, _
        bo_http_connectTimeout, _
        bo_http_sendTimeout, _
        bo_http_receiveTimeout, _
        bo_api_http_server, _
        bo_err_ret, _
        bo_url, _
        bo_request_string, _
        bo_dp, _
    ...
    bo_sql="exec disaster..spEdit_APSCRN_BO_API_Log @application_id=" & fnBoCleanString(bo_application_id,"numeric") & ", @from_ofs_api_log_id=" & fnBoCleanString(from_ofs_api_log_id,"numeric") & ", @usr_id=" & fnBoCleanString(session("usr_id"),"numeric")
    set bo_rs=bo_dp.GetData(bo_sql)
    bo_api_log_id=bo_rs("id")

    bo_sql="exec disaster..spGet_Apscrn_BO_Application @ofs_app_id=" & fnBoCleanString(bo_application_id,"string")
    set bo_rs=bo_dp.GetData(bo_sql)

    if bo_rs.eof then
        bo_return_post_errors=bo_return_post_errors & fnBoWrapError("cannot find application " & bo_application_id & ".")

        bo_sql="exec disaster..spEdit_APSCRN_BO_API_Log @id=" & bo_api_log_id & ", @bo_xml=null, @process_return=" & fnBoCleanString(fnBoWrapReturn(bo_return_post_errors,false),"string")
        bo_dp.Execute bo_sql
    else
        bo_qs_hdr_id=bo_rs("header_id")
        bo_qs_detail_id=bo_rs("detail_id")
        bo_url=bo_rs("bo_url")
        bo_request_string=bo_rs("bo_authentication")
        ...
        if isnull(bo_url) or isnull(bo_request_string) then
            ...
            if not bo_call_valid then
                bo_sql="exec disaster..spEdit_APSCRN_BO_API_Log @id=" & bo_api_log_id & ", @bo_xml=null, @process_return=" & fnBoCleanString(fnBoWrapReturn(bo_return_post_errors,false),"string") & ", @hdr_id=" & bo_qs_hdr_id & ", @detail_id=" & bo_qs_detail_id
            else
                bo_sql="exec disaster..spEdit_APSCRN_BO_API_Log @id=" & bo_api_log_id & ", @bo_xml=" & fnBoCleanString(bo_xml_return.xml,"string") & ", @process_return=" & fnBoCleanString(fnBoWrapReturn(bo_return_post_errors,true),"string") & ", @hdr_id=" & bo_qs_hdr_id & ", @detail_id=" & bo_qs_detail_id
            end if
