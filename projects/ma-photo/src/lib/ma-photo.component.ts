...
@Component({
  ...
  <i *ngIf="tempList.length > 0" class="ico-edit-blue pull-right" (click)="openPhotosModal()"></i>
  <div *ngIf="tempList.length > 0" (click)="openPhotosModal()"><a href="javascript:void(0)">View/Edit Photos</a></div>
  <div *ngIf="tempList.length === 0" (click)="openPhotosModal()"><a href="javascript:void(0)" class="">Add Photos</a></div>
  <ma-photo-modal *ngIf="displayPhotosModal && !disabled" [(ngModel)]="ngModel" [multiple]="multiple" [isActive]="isActive" [readOnly]="readonly" [disabled]="disabled"
    [required]="required" [id]="id" [name]="name" (close)="closePhotosModal($event)" (ngModelChange)="ngModelChangeList($event)"></ma-photo-modal>
</ng-container>
<ng-template #simple>
...
<div *ngIf="displayPhotos.length > 0" (click)="openPhotosModal()"><a href="javascript:void(0)">View/Edit Photos</a></div>
<div *ngIf="displayPhotos.length === 0" (click)="openPhotosModal()"><a href="javascript:void(0)" class="">Add Photos</a></div>
<ma-photo-modal *ngIf="displayPhotosModal && !disabled" [(ngModel)]="ngModel" [multiple]="multiple" [isActive]="isActive" [readOnly]="readonly" [disabled]="disabled"
